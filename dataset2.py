...
class NMT_Dataset(Dataset):
    def __init__(self, input_tensors, output_tensors,source_len_tensor):
        self.input_tensors = input_tensors
        self.output_tensors = output_tensors
        self.source_len_tensor = source_len_tensor

    def __len__(self):
        return len(self.input_tensors)

    def __getitem__(self, idx):
        return self.input_tensors[idx], self.output_tensors[idx], self.source_len_tensor[idx]
...
def read_languages(data_type, use_saved_vocab): ## train / dev te
    ...
    with open(en_path, 'r', encoding = 'utf-8') as f1, open(de_path,'r', encoding = 'utf-8') as f2:
        ...
        if use_saved_vocab == True:
            with open('vocab_local/local_input_vocab.pkl', 'rb') as f1, open('vocab_local/local_output_vocab.pkl', 'rb') as f2:
...
def filter_pairs(pairs,max_len,min_len):
    return [pair for pair in pairs if filter_pair(pair,max_len,min_len)]
...
def prepare_data(data_type, max_len, min_len,use_saved_vocab): ## train / dev / test
    ...
    pairs = filter_pairs(pairs,max_len,min_len)
    ...
    if use_saved_vocab == True:
        ...
        # 저장
        with open('vocab_local/local_input_vocab.pkl', 'wb') as f1, open('vocab_local/local_output_vocab.pkl', 'wb') as f2:
...
def tensor_from_sentence(vocab, sentence, is_output, max_len, data_type):
    ...
    if is_output:
        ...
        if len(_indexes) < max_len: #max_len
            length_of_index = len(_indexes)
            _indexes += [vocab.pad_token] * (max_len - len(_indexes))
        else:
            _indexes = _indexes[:max_len]
            length_of_index = len(_indexes)
            
    return torch.tensor(_indexes, dtype = torch.long), torch.tensor(length_of_index, dtype = torch.long)
...
def tensor_from_pair(pairs, input_vocab, output_vocab, max_len, data_type):
    ...
    source_tensors = torch.zeros((batch_size), dtype=torch.long)
    output_tensors = torch.zeros((batch_size, max_len+1), dtype=torch.long)

    for i, pair in enumerate(pairs):
        input_indexes,source_len = tensor_from_sentence(input_vocab, pair[0], False, max_len, data_type)
        output_indexes,_ = tensor_from_sentence(output_vocab, pair[1], True, max_len, data_type)

        # 미리 할당한 텐서에 각 문장 인덱스를 저장
        input_tensors[i] = input_indexes
        output_tensors[i] = output_indexes
        source_tensors[i] = source_len

    return input_tensors, output_tensors,source_tensors